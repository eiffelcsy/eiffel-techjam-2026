...
from eval.config import DegradeConfig, DetectorConfig, RunConfig, load_run_config
from load_data.config import DatasetConfig
from load_data.manifest import build_manifest, load_manifest
from preprocessing.degrade.conditions import build_conditions, load_grid
from eval.detectors import build_detector
from eval.report import headline_table, load_results
from eval.runner import run_eval
from tests.eval.fixtures import SyntheticSource

GRID_FILE = "preprocessing/configs/degradations.yaml"
N_PER_CLASS, N_REPLICATES = 8, 2
DETECTOR_CONFIGS = sorted(Path("eval/configs/detectors").glob("*.yaml"))
DATASET_CONFIGS = sorted(Path("load_data/configs/datasets").glob("*.yaml"))
...
def result(tmp_path_factory):
    ...
    cfg = RunConfig(
        ...
        detectors=[DetectorConfig(
            name="stub", target="tests.eval.fixtures.StubDetector", args={}, device="cpu",
        )],
...
def test_result_matches_the_schema_report_reads(result):
    ...
    assert payload["detector_spec"]["target"] == "tests.eval.fixtures.StubDetector"
...
def test_a_zoo_of_detectors_lands_in_one_comparable_table(result, tmp_path):
    ...
    cfg = RunConfig(
        run_id="zoo",
        detectors=[
            DetectorConfig(name="stub-a", target="tests.eval.fixtures.StubDetector",
                           args={}, device="cpu"),
            DetectorConfig(name="stub-b", target="tests.eval.fixtures.StubDetector",
...
def test_csv_metadata_source_selects_by_path_and_labels_by_column(tmp_path):
    ...
    from load_data.sources import CsvMetadataSource
...
def test_stratified_source_samples_every_group_in_proportion(tmp_path):
    ...
    """
    from load_data.sources import StratifiedCsvSource
...
def test_run_config_takes_one_detector_or_many(tmp_path):
    ...
    def write(body: str) -> Path:
        ...
        path.write_text(
            f"run_id: r\ndatasets: [load_data/configs/datasets/wildfake_train.yaml]\n{body}"
        )
        return path
