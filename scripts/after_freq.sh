...
from train.config import load_cache_config, load_enrich_config, load_probe_config
...
cfgs = [
    load_probe_config("train/configs/probe/dinov3_wildfake_multiscale.yaml").crop,
    load_cache_config("train/configs/cache/dinov3_multiscale.yaml").crop,
    load_cache_config("train/configs/cache/dinov3_multiscale_val.yaml").crop,
    load_cache_config("train/configs/cache/wildfake_freq.yaml").crop,
    load_cache_config("train/configs/cache/wildfake_freq_val.yaml").crop,
    load_enrich_config("train/configs/train/dinov3_enrich.yaml").crop,
]
...
if step "P3' render the crop-era SPATIAL caches -- train + val"; then
  ...
  render train/configs/cache/dinov3_multiscale.yaml
  render train/configs/cache/dinov3_multiscale_val.yaml
fi
...
if step "S0' stage 1 under the crop protocol, five seeds -- and the NOISE FLOOR"; then
  ...
  # an arm that shifts a metric by less than about twice that spread has shifted
  # nothing -- and every ablation in step 9 is read against this number.
  stage1 dinov3_multiscale train/configs/train/dinov3_multiscale.yaml
  for s in 1 2 3 4; do
    stage1 "dinov3_multiscale_s$s" train/configs/train/dinov3_multiscale.yaml \
...
if step "render the FREQUENCY caches -- features AND the DCT view, ~31 GB"; then
  ...
  # setting.
  render train/configs/cache/wildfake_freq.yaml
  render train/configs/cache/wildfake_freq_val.yaml
fi
...
if step "stage 2 -- the enricher, frozen head AND fine-tuned head (E14)"; then
  ...
  enrich dinov3_enrich    train/configs/train/dinov3_enrich.yaml
  enrich dinov3_enrich_ft train/configs/train/dinov3_enrich.yaml --finetune-head
fi
...
if step "THE HEADLINE -- six arms x two eval arms, one condition lattice"; then
  ...
  if have "$FIRST"; then note "already scored -- skipping"
    ...
    "$PY_ABS" scripts/run_eval.py \
        --config eval/configs/runs/dinov3_poc_freq.yaml || die "the headline run failed"
  fi
fi
...
if step "the ablations -- E11 (band split), E13 (coefficients), E15 (anchor)"; then
  if (( SKIP_ABL )); then note "--skip-ablations"; else
    ...
    ENRICH=train/configs/train/dinov3_enrich.yaml
