...
# train/configs/probe/dinov3_wildfake_multiscale.yaml. Refuses to start
# otherwise -- CropConfig raises on `enabled: true` with no `s_max`, which is
# the point.
#
...
for ds in wildfake_coco_dalle3 wildfake_train_val; do
  ...
  "$PY_ABS" scripts/shortcut_baseline.py \
      --dataset "load_data/configs/datasets/${ds}.yaml" \
      --out "$out" || die "shortcut baseline failed on $ds"
done
...
if have "$RT"; then
  ...
  "$PY_ABS" scripts/roundtrip_check.py \
...
BASE="results/dinov3_poc_baseline_arms__dinov3-wildfake-crop200__wildfake-coco-dalle3.json"
if have "$BASE"; then
  note "already scored -- skipping"
else
  "$PY_ABS" scripts/run_eval.py \
      --config eval/configs/runs/dinov3_poc_baseline_arms.yaml || die "P2' failed"
fi
...
$(printf '\033[1m== done. What you now have, and how to read it\033[0m')
  ...
  results/dinov3_poc_baseline_arms__*.json
