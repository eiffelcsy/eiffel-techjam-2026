...
#
# Run it from grace_adapter/ (or anywhere -- it cd's to its own parent). Leave it
# in its own terminal; the fetch is ~17 hours and this sits on top of it.
#
# WHAT IT DOES
#
...
# whatever form the shell itself can use. Absolute, because every step below
# runs its command from inside ../eval_pipeline.
...
Rerun it before this script:
    cd ../eval_pipeline && $PY_ABS scripts/fetch_wildfake_train.py --resume
...
if [[ -e "$MANIFEST" ]]; then
  ...
  ( cd ../eval_pipeline && "$PY_ABS" scripts/build_manifest.py \
      --config configs/datasets/wildfake_train.yaml ) || die "manifest build failed"
fi
...
# `tests/test_configs.py::test_every_crop_config_agrees_on_the_range` is the
# check that they still match afterwards.
CROP_CONFIGS=(
  "$PROBE"
