...
#
# Run it from grace_adapter/ (or anywhere -- it cd's to its own parent).
#
...
# produce the headline number and the noise floor it has to be read against.
# Steps 12-16 are attribution: which parts of the objective earn their place.
# Steps 17-22 are the two hard questions (what the method destroys, whether
# localization helps) and the slow control.
...
evalrun() {                      # evalrun <result-json> <run config>
  ...
  ( cd "$HARNESS" && "$PY_ABS" scripts/run_eval.py --config "$2" )
}
...
if step "P2  the baseline -- the retention curve everything is measured against"; then
  ...
  evalrun "$BASE" configs/runs/dinov3_poc_baseline.yaml
fi
...
if step "E1  THE GATE -- the null adapter must reproduce the baseline exactly"; then
  ...
  evalrun "$R/dinov3_poc_identity__dinov3+identity__wildfake-coco-dalle3.json" \
          configs/runs/dinov3_poc_identity.yaml
...
if step "P3  render the feature caches -- train + both val sets"; then
  ...
  #
  # The two val caches are NOT optional: every configs/train/dinov3_*.yaml names
  # them in `val_cache_dirs` and train_adapter.py opens them in its first few
  # statements, so a missing one fails at second zero rather than after the run.
  # Separate out_dir each, because build_cache.py names its root after the
...
if step "S0  the reference arm, five seeds -- and the NOISE FLOOR"; then
  ...
  stage1 dinov3_clean configs/train/dinov3_clean.yaml
  for s in 1 2 3 4; do
    stage1 "dinov3_clean_s$s" configs/train/dinov3_clean.yaml --seed "$s" --run-id "dinov3_clean_s$s"
  done
...
if step "E5a stage 2 -- the supervised discrepancy head, adapter frozen"; then
  ...
  stage2 dinov3_disc configs/train/dinov3_discrepancy.yaml
fi
...
if step "E5b the beta sweep -- bound the fusion over ALL weightings"; then
  ...
  if have results/dinov3_beta_sweep.json; then note "beta sweep exists -- skipping"
    ...
    "$PY_ABS" scripts/sweep_beta.py configs/train/dinov3_discrepancy.yaml \
...
if step "E5c THE HEADLINE -- score +grace and +grace-d through the harness"; then
  ...
  evalrun "$R/dinov3_poc_grace__dinov3+grace__wildfake-coco-dalle3.json" \
          configs/runs/dinov3_poc_grace.yaml
...
if step "E2  the clean teacher -- is it the mechanism, or self-distillation?"; then
  ...
  # result. tests/test_configs.py pins the pair at one key apart.
  stage1 dinov3_degraded configs/train/dinov3_degraded.yaml
fi
...
if step "E3  the loss ablation -- does the Jacobian weighting earn its place?"; then
  ...
  stage1 dinov3_plain_mse configs/train/dinov3_plain_mse.yaml
fi
...
if step "E8  the gate -- is it learning, or is AdamW decaying it open?"; then
  ...
  stage1 dinov3_gate_nodecay configs/train/dinov3_gate_nodecay.yaml
...
if step "E9a hyperparameters -- the loss-weight ratio (the only live axis)"; then
  ...
  stage1 dinov3_sweep_wratio_4    configs/train/dinov3_sweep_wratio_4.yaml
fi
...
if step "E9b hyperparameters -- capacity and gate init"; then
  ...
  stage1 dinov3_sweep_nblocks_2      configs/train/dinov3_sweep_nblocks_2.yaml
  stage1 dinov3_sweep_bottleneck_256 configs/train/dinov3_sweep_bottleneck_256.yaml
  stage1 'dinov3_sweep_gate_-3'      'configs/train/dinov3_sweep_gate_-3.yaml'
  "$PY_ABS" scripts/seed_stats.py dinov3_clean 'dinov3_clean_s*' \
...
if step "E4  the erasure trade-off -- does restoring destroy the evidence?"; then
  ...
  for ck in "${cks[@]}"; do
    ...
    stage2 "$rid" configs/train/dinov3_discrepancy.yaml \
...
if step "E7b the ladder -- the 12-epoch arm, and the tap_dim sweep"; then
  ...
  stage1 dinov3_ladder_final    configs/train/dinov3_ladder_final.yaml
  stage1 dinov3_sweep_tapdim_32  configs/train/dinov3_sweep_tapdim_32.yaml
  stage1 dinov3_sweep_tapdim_128 configs/train/dinov3_sweep_tapdim_128.yaml
  stage1 dinov3_sweep_tapdim_256 configs/train/dinov3_sweep_tapdim_256.yaml
  "$PY_ABS" scripts/seed_stats.py dinov3_clean 'dinov3_clean_s*' \
...
if step "E7d the ladder -- score it on the same denominator as everything else"; then
  evalrun "$R/dinov3_poc_ladder__dinov3+grace-ladder__wildfake-coco-dalle3.json" \
          configs/runs/dinov3_poc_ladder.yaml
...
if step "E6  cached vs live -- is the finite augmentation set being exploited?"; then
  if (( SKIP_SLOW )); then note "--skip-slow: not running the live control"
    ...
    #                     corruptions, and n_epochs in configs/cache/dinov3.yaml
    #                     has to go up, which means a re-render.
    stage1 dinov3_live configs/train/dinov3_live.yaml
...
1. IS THERE A PROBLEM AT ALL?
   eval_pipeline/results/dinov3_poc_baseline__*.json -> levels.*.retention
