"""Would weighting the auxiliary head more push GRACE-D past retention 1.0?
    ...
    python scripts/sweep_beta.py configs/train/dinov3_discrepancy.yaml --run disc_gate_ctrl
...
do at ANY weighting, so:
  ...
  * peak retention > 1.0 => the drift carries evidence the clean image does not,
    exactly as `grace.models.discrepancy` claims, and the training objective is
...
def main():
    ...
    from grace.config import DiscrepancyConfig
    fused = FusedHead(
